...
bool can_receive(can_message_t *msg, uint32_t timeout)
{
    ...
    while ((0 - start_time) < timeout)
    { // Placeholder cho systick
        if (msg_received)
        {
            *msg = rx_buffer;
            msg_received = false;
 ...
 * @note Bank 0: Lọc FB_SLAVE1 (0x301) - 32-bit list mode
 * @note Bank 1: Lọc FB_SLAVE2 (0x302) - 32-bit list mode
 */
void can_master_setup_filters(can_feedback_t fb_slave1, can_feedback_t fb_slave2)
{
    ...
    // Bank 0: FB_SLAVE1 (0x301)
    CAN1->FS1R &= ~CAN_FS1R_FSC0;  // 16-bit scale
    CAN1->FM1R |= CAN_FM1R_FBM0;   // List mode
    CAN1->FFA1R |= CAN_FFA1R_FFA0; // Assign to FIFO 1
    CAN1->sFilterRegister[0].FR1 = ((fb_slave1 & 0x7FF) << 5);
    CAN1->sFilterRegister[0].FR2 = 0;
    CAN1->FA1R |= CAN_FA1R_FACT0; // Activate filter 0

    // Bank 1: FB_SLAVE2 (0x302)
    CAN1->FS1R &= ~CAN_FS1R_FSC1;  // 16-bit scale
    CAN1->FM1R |= CAN_FM1R_FBM1;   // List mode
    CAN1->FFA1R |= CAN_FFA1R_FFA1; // Assign to FIFO 1
    CAN1->sFilterRegister[1].FR1 = ((fb_slave2 & 0x7FF) << 5);
    CAN1->sFilterRegister[1].FR2 = 0;
    CAN1->FA1R |= CAN_FA1R_FACT1; // Activate filter 1
 ...
 * 
 * @note Bank 0: Mask mode - Chấp nhận 0x100-0x10F (STATUS_ALL, SETPOINT_ALL, STATUS_SLAVE1/2)
...
void can_slave_setup_filters(can_command_t cmd_start, can_command_t cmd_stop, can_command_t cmd_setpoint, can_command_t cmd_status)
{
       ...
       // Bank 0: Mask mode cho range 0x100-0x10F (32-bit scale)
       // Accepts: CMD_SETPOINT_ALL (0x100), CMD_STATUS_ALL (0x101), CMD_STATUS_SLAVE1 (0x102), CMD_STATUS_SLAVE2 (0x103)
       CAN1->FS1R |= CAN_FS1R_FSC0;   // 32-bit scale
       CAN1->FM1R &= ~CAN_FM1R_FBM0;  // Mask mode
       CAN1->FFA1R |= CAN_FFA1R_FFA0; // Assign to FIFO 1
       // ID Filter: 0x100 (0001 0000 0000), Mask: 0x7F0 (lọc bits 10-4, chấp nhận 0x100-0x10F)
       CAN1->sFilterRegister[0].FR1 = (0x100 << 21);  // Filter ID: 0x100
       CAN1->sFilterRegister[0].FR2 = (0x7F0 << 21);  // Mask: 0x7F0 (accept 0x100-0x10F)
       CAN1->FA1R |= CAN_FA1R_FACT0; // Activate filter 0
    
       // Bank 1: List mode cho START và STOP commands (16-bit scale)
       CAN1->FS1R &= ~CAN_FS1R_FSC1; // 16-bit scale
       CAN1->FM1R |= CAN_FM1R_FBM1;  // List mode
       CAN1->FFA1R |= CAN_FFA1R_FFA1; // Assign to FIFO 1
       CAN1->sFilterRegister[1].FR1 = ((cmd_start & 0x7FF) << 21) | ((cmd_stop & 0x7FF) << 5);
       CAN1->sFilterRegister[1].FR2 = 0; // Not used
       CAN1->FA1R |= CAN_FA1R_FACT1; // Activate filter 1
    
       // Bank 2: List mode cho SETPOINT command riêng (16-bit scale)
       CAN1->FS1R &= ~CAN_FS1R_FSC2; // 16-bit scale
       CAN1->FM1R |= CAN_FM1R_FBM2;  // List mode
       CAN1->FFA1R |= CAN_FFA1R_FFA2; // Assign to FIFO 1
       CAN1->sFilterRegister[2].FR1 = ((cmd_setpoint & 0x7FF) << 21);
       CAN1->sFilterRegister[2].FR2 = 0; // Not used
       CAN1->FA1R |= CAN_FA1R_FACT2; // Activate filter 2
